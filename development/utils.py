...
def build_and_solve_multi_tier_ttr(dataset: dict, disrupted: list[str], ttr: float) -> pd.DataFrame:
    """
    ...
    Returns:
    ConcreteModel: The solved Pyomo model.
    """
    # Prepare your data
    data = {
        ...
        'd': dataset['d'],  # demand for j per TTR
        'c': dataset['c'],  # plant capacity per TTR
    ...
    # Decision variables
    m.u = pyo.Var(m.NODES, domain=pyo.NonNegativeIntegers)  # production quantity of node i
    m.l = pyo.Var(m.V, domain=pyo.NonNegativeIntegers)  # lost volume of product j
    m.y_index = pyo.Set(within=m.U * m.NODES, initialize=lambda mdl: [
        (i, j) for i in mdl.U for j in mdl.N_plus[i]
    ])
    m.y = pyo.Var(m.y_index, domain=pyo.NonNegativeIntegers)  # y is only needed for (i,j) pairs that actually make sense
